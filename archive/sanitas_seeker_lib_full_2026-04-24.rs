// ============================================================
// Sanitas Seeker — application program (fitness / SBT / pools / news).
// Program upgrades are gated by the separate `upgrade_governor` program
// (see programs/governor). This binary intentionally does not self-upgrade.
// ============================================================
...
// ==================== DATA STRUCTURES ====================
#[derive(AnchorSerialize, AnchorDeserialize, Clone, serde::Serialize, serde::Deserialize, Default)]
pub struct FurtherReadingItem {
    #[serde(default)]
    pub label: String,
    #[serde(default)]
    pub url: String,
...
/// One daily issue: workout explainer video + optional AI "desk" health copy + optional outbound links.
/// `post_*` are legacy; app no longer embeds X in WebView. Serde accepts camelCase from older tooling.
#[derive(AnchorSerialize, AnchorDeserialize, Clone, serde::Serialize, serde::Deserialize)]
pub struct NewsThread {
    pub id: String,
    pub timestamp: i64,
    pub date: String,
    #[serde(default, alias = "videoDate")]
    pub video_date: String,
    /// Workout / featured section
    pub title: String,
    pub content: String,
    #[serde(default, alias = "postUrls")]
    pub post_urls: Vec<String>,
    #[serde(default, alias = "postIds")]
    pub post_ids: Vec<String>,
    #[serde(default, alias = "videoUrl")]
    pub video_url: String,
    #[serde(default, alias = "videoTag")]
    pub video_tag: String,
    /// Optional: overrides title in UI for the workout block (falls back to `title`)
    #[serde(default, alias = "workoutTitle")]
    pub workout_title: String,
    /// AI / persona health explainer (not medical advice; educational)
    #[serde(default, alias = "healthReviewTitle")]
    pub health_review_title: String,
    #[serde(default, alias = "healthReviewBody")]
    pub health_review_body: String,
    /// Same artwork style as SBT / brand when hosted on Arweave or static CDN
    #[serde(default, alias = "personaImageUrl")]
    pub persona_image_url: String,
    #[serde(default, alias = "furtherReading")]
    pub further_reading: Vec<FurtherReadingItem>,
}
